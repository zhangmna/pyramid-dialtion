...
class Session:
    ...
    def inf_batch(self, name, batch):
        ...
        with torch.no_grad():
            O_Rs,path1, path3, path5, pool1 = self.net(O)
        ...
        return O_Rs[-1],path1, path3, path5, pool1
...
def run_show(ckp_name):
    ...
    for i, batch in enumerate(dt):
        logger.info(i)
        imgs,path1, path3, path5, pool1= sess.inf_batch('test', batch)
        sess.save_image(i, imgs)
        sess.save_image_feature('pool1',path1)
        sess.save_image_feature('pool3',path3)
        sess.save_image_feature('pool5',path5)
        sess.save_image_feature('final', pool1)
